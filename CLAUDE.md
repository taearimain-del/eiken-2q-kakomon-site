...
## マークシート機能
- マークシート機能は2026-09-19に `claude-workspace/英検マーク式演習/2級/kakomon.html`
  （筆記・リスニング・Writingを年度回ごとに1ページへ統合したツール）へ移管した。
  音声ファイルの実体はこのリポジトリ（`audio/`）が正で、統合ツール側の `audio/` は
  デプロイ用にコピーしたもの（`claude-workspace`の`.gitignore`でコミット対象外にしている）。
  音声を追加・入れ替えたら統合ツール側にもコピーし直すこと。
- このリポジトリ固有の `listening-marksheet.html` は撤去済み。`index.html` から
  統合ツールへのリンクを貼っている。
